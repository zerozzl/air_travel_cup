...
from ranker.dataloader import read_data, create_data_from_ranker, RankColBertDataset
from models.Ranker import ColBert
...
def get_dataset(args, epoch, tokenizer, train_data, test_data):
    ...
    train_dataset = RankColBertDataset(train_data, tokenizer, args.max_question_len, args.max_context_len,
                                       epoch=epoch, negative_batch_size=negative_batch_size, with_sep=False)
    test_dataset = RankColBertDataset(test_data, tokenizer, args.max_question_len, args.max_context_len,
                                      epoch=epoch, negative_batch_size=negative_batch_size, with_sep=False)
...
def data_collate_fn(data):
    ...
    question_tokens = [torch.LongTensor(np.array(item[0])) for item in data[:, 0]]
    question_tokens = rnn.pad_sequence(question_tokens, batch_first=True)

    question_tokens_type_id = [torch.LongTensor(np.array(item[1])) for item in data[:, 0]]
    question_tokens_type_id = rnn.pad_sequence(question_tokens_type_id, batch_first=True)

    question_tokens_mask = (question_tokens > 0).int()
    question_list = [question_tokens, question_tokens_type_id, question_tokens_mask]

    positive_tokens = [torch.LongTensor(np.array(item[0])) for item in data[:, 1]]
    positive_tokens = rnn.pad_sequence(positive_tokens, batch_first=True)

    positive_tokens_type_id = [torch.LongTensor(np.array(item[1])) for item in data[:, 1]]
    ...
    return question_list, positive_list, negative_list
...
def train(args, dataset, dataloader, model, optimizer, lr_scheduler):
    ...
    for batch, data in enumerate(dataloader):
        ...
        question_list, positive_list, negative_list = data

        question_list = [item.cpu() if args.use_cpu else item.cuda() for item in question_list]
        ...
        question_embeds = model(question_list[0], question_list[1], question_list[2])
        positive_embeds = model(positive_list[0], positive_list[1], positive_list[2])

        logits = []
        positive_logits = model.score(question_embeds, positive_embeds)
        for idx in range(len(negative_list)):
            negative = negative_list[idx]
            negative_embeds = model(negative[0], negative[1], negative[2])
            negative_logits = model.score(question_embeds[idx].unsqueeze(0), negative_embeds)
            logits.append(torch.cat([positive_logits[idx].unsqueeze(0), negative_logits], dim=0))
    ...
    with torch.no_grad():
        for batch, data in enumerate(dataloader):
            ...
            for idx in range(len(positive_logits)):
                ...
                logits = torch.cat([positive_logits[idx].unsqueeze(0), negative_logits], dim=0)
...
def main(args):
    ...
    if args.pretrained_model_path is not None:
        ...
        model = ColBert('%s/config.json' % args.pretrained_bert_path,
                        '%s/pytorch_model.bin' % args.pretrained_bert_path,
                        args.output_size, args.bert_freeze)
        model = model.cpu() if args.use_cpu else model.cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_path', type=str,
                        default='../../runtime/ranker/colbert')
