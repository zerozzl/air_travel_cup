...
from utils.data_utils import TOKEN_CLS, TOKEN_SEP, TOKEN_MASK, COLBERT_Q_SPE, COLBERT_D_SPE
...
def check_question(tokenizer, model, question, paragraphs, accept_size,
    ...
    question_tokens = torch.LongTensor(question_tokens).unsqueeze(0)
    question_tokens_type_id = torch.LongTensor(question_tokens_type_id).unsqueeze(0)
    question_tokens_mask = (question_tokens > 0).int()

    question_tokens = question_tokens.cpu() if use_cpu else question_tokens.cuda()
    question_tokens_type_id = question_tokens_type_id.cpu() if use_cpu else question_tokens_type_id.cuda()
    question_tokens_mask = question_tokens_mask.cpu() if use_cpu else question_tokens_mask.cuda()
    question_embeds = model(question_tokens, question_tokens_type_id, question_tokens_mask)
    ...
    for batch_idx in range(batch_num):
        ...
        for para in batch_paras:
            ...
            para_tokens = [TOKEN_CLS, COLBERT_D_SPE] + para_tokens
            if max_context_len > 0:
                para_tokens = para_tokens[:max_context_len]
        ...
        b_embeds = model(b_tokens, b_tokens_type_id, b_tokens_mask)
        b_logits = model.score(question_embeds, b_embeds)
        b_logits = b_logits.cpu().numpy()
